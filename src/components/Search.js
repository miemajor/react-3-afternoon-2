...
class Search extends Component{
    ...
    search(event){
        ...
        axios.get(`/api/${this.state.searchType}?q=${searchTerm}`).then(results=>{
            if(searchType==='blogs'){
                this.props.history.push(makeQuery('/search?',{q:searchTerm,type:searchType}))
                this.setState({'blogResults':results.data, 'userResults':[]})
            }else{
                this.props.history.push(makeQuery('/search?',{q:searchTerm,type:searchType}))
                this.setState({'blogResults':[],'userResults':results.data})
            }
        }).catch(console.log);
    }
